...
# readonly SCRIPT_NS="skip_trace_proc"
readonly SRC_DIR="./skip_trace"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/skip_trace"
readonly OUTPUT_MD_BASENAME="skip_trace.flat.md"

# Copy directory safely
skip_trace_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/skip_trace
skip_trace_proc::run_strip_docs() {
  local dir
...
# in the generated Markdown (skip_trace.flat.md). Other code fences remain untouched.
skip_trace_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  skip_trace_proc::copy_to_tmp
  skip_trace_proc::run_strip_docs
  skip_trace_proc::confirm "Run coderoller-flatten-repo?"
  skip_trace_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  skip_trace_proc::clean_flat_md_trailing_double_hash
  skip_trace_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  skip_trace_proc::cleanup_tmp
}
